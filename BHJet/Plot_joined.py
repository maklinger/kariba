...
for i in range(nzones - 1):
    ...
    for k in range(int(size_com_arr[i])):
        nu_compton[k] = Compton_zones.T[0][totindex2 + k] / kevconv
        lnu_compton[k] = Compton_zones.T[1][totindex2 + k]
    totindex1 = totindex1 + int(size_cyclo_arr[i])
    totindex2 = totindex2 + int(size_com_arr[i])
    if (i % 1) == 0:
        ax2.plot(
...
ax2.plot(
    Presyn.T[0] / kevconv, Presyn.T[1], linewidth=2.5, color="cyan", zorder=nzones + 1
)
ax2.plot(
    Postsyn.T[0] / kevconv,
    Postsyn.T[1],
...
ax2.plot(
    Precom.T[0] / kevconv, Precom.T[1], linewidth=2.5, color="blue", zorder=nzones + 1
)
ax2.plot(
    Postcom.T[0] / kevconv,
    Postcom.T[1],
...
ax2.plot(
    Total.T[0] / kevconv, Total.T[1], linewidth=1.5, color="black", zorder=nzones + 1
)
...
if plotcheck >= 2:  # We create the colorbar:
    ...
    sm.set_array(colors)
    bar = fig.colorbar(sm, pad=0.10, ax=[ax1, ax2], ticks=[0.01, 0.25, 0.5, 0.75, 0.99])
    bar.set_label(r"$log_{10}(z)\,(r_g$)", fontsize=22)
...
if plotcheck >= 2:
    ...
    bar2 = fig2.colorbar(
        sm2, pad=0.10, ax=[ax3, ax4], ticks=[0.01, 0.25, 0.5, 0.75, 0.99]
    )
    bar2.set_label(r"$log_{10}(z)\,(r_g$)", fontsize=22)
    bar2.ax.set_yticklabels(zheight, fontsize=15)

plt.autoscale(False)
plt.savefig("joined.pdf")
# plt.show()
