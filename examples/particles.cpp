...
int main() {
    ...
    // effective expansion speed to define the adiabatic timescale, and
    // non-thermal index. These parameter values roughly mimic the corona of a
    // BHB
    double R = 3.e7;
    double Te = 511.;
    double gmax = 1.e3;
    ...
    // with set_p. Here it is called to a fixed maximum Lorenz factor. After
    // that, specify the fraction of particles in non-thermal particles, the
    // index of the non-thermal particles, and then calculate the normalisation
    // of the distributions for a given number density, which here is just set
    // to unity. The full distribution before cooling can only be calculated
    // correctly with set_ndens() after all these setters have been called
    // correctly Finally, after all this is done, the steady-state distribution
    // including cooling can be computed by passing the energy density of
    // external photons (here set to 0), the total number density of the final
    // distribution, the magnetic field, the radius of the emitting region and
    // its expansion speed
    ...
    // the kappa index needs to be increased by 1 to match the same slope
    ...
    // momentum and Lorenz factor arrays you need to set the temperature, and
    // before calculating the number density arrays you need to specify the
    // total number density. After that, it is possible to calculate the average
    // Thermal momentum to set the break momentum in the broken power-law.
    ...
    // For the broken power-law, first set up the slopes before and after the
    // break, then set the momentum and Lorenz factor arrays setting the minimum
    // of the distribution to be one tenth of the thermal break, and again
    // hard-coding the maximum. After that, as usual, you need to specify the
    // number density before calculating the number density of the uncooled
    // distribution, and you need to pass photon energy density/particle number
    // density/magnetic field/radius/expansion speed for the cooled distribution
