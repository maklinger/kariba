...
// Compton classes for a range of electrons temperatures and optical depths. The
// output is then compared to CompPS in the python plotting script.
...
int main() {
    ...
    // Disk parameters: black hole mass and corresponding Eddington luminosity
    ...
    // Set up the optical depths, temepratures, number densities, and sizes for
    // all our coronae. Note that because the seed photon energy density in
    // Kariba is calculated in simplified fashion at the center of the emititng
    // region and neglecting GR, in this case the radii of the corona should be
    // thought of less as physical sizes and more as normalisations
    double Tau[3] = {2.6, 0.76, 0.19};
    double Te[2] = {90., 900.};
    double R[6] = {45. * Rg, 75. * Rg, 110. * Rg, 4. * Rg, 25. * Rg, 130. * Rg};
    double ndens[6];

    // For each combination of temperature and optical depth, set up the number
    ...
    // (as many other classes, see later), because for a disk it's always taken
    // to be 50. Also note that you ALWAYS should specify the  inner and outer
    // radii, THEN the luminosity. This is because the innermost radius is
    // necessary to calculate the temperature profile, starting from the
    ...
    // the arrays need to be called in the appropriate order. You can only set
    ...
    // Retrive the bounds of the electron distribution and set up the
    // interpolation of the arrays. Both of these are needed as arguments for
    // the Compton class.
    ...
    // both the size of the output arrays, and of the seed photon arrays, which
    // for a disk is always set to 50. You then need to specify, in no
    // particular order, the range over which you want the emission to be
    // computed, what the geometry of the emitting region is and whehter it is
    // beamed. After that, specify the temperature and number density of the
    // radiating particles. This is required to calculate the radiative transfer
    // corrections to the spectrum, if multiple scatterings are computed. Here,
    // 20 scatterings are hard coded for illustration purposes; the class
    // constructor by default sets 20 scatterings, and if the optical depth is
    // below 0.05 this is set automatically to 1 by set_tau to save
    // computational time. Finally, specify the seed photon field - including
    // only disk photons here, by passing an array of energies, the innermost
    // temperature of the disk, its inner and outer radii, the scale height of
    // the disk (although it has only a minor effect on the spectra), and the
    ...
    // objects we could also just update all the parameters of of the initial
